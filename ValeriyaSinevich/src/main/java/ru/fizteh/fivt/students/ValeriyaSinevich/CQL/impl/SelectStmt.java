...
import ru.fizteh.fivt.students.ValeriyaSinevich.CQL.Aggregates;
import ru.fizteh.fivt.students.ValeriyaSinevich.CQL.Query;
        ...
        if (groupByFunctions != null) {
            ...
            for (List<T> group : grouppedList) {
                for (int i = 0; i < functions.length; i++) {
                    if (functions[i] instanceof  Aggregates.AggregationFunction) {
                        arguments[i] = ((Aggregates.AggregationFunction) functions[i]).apply(group);
                    } else {
            ...
            for (T element : this.list) {
                for (int i = 0; i < functions.length; i++) {
                    ...
                    if (functions[i] instanceof  Aggregates.AggregationFunction) {
                        ...
                        currArg.add(element);
                        arguments[i] = ((Aggregates.AggregationFunction) functions[i]).apply(currArg);
                    } else {
                        arguments[i] = functions[i].apply(element);
                    }
                    clazzes[i] = arguments[i].getClass();
